...
(IN p_id INTEGER , IN p_pin INTEGER, IN p_valid INTEGER, OUT sql_code INTEGER, OUT err_msg CHAR(100))
LANGUAGE SQL
  BEGIN
    IF p_gender != 'M' AND p_gender != 'F' THEN
      SET sql_code = -100;
...
(IN p_id INTEGER, IN p_bal INTEGER, IN p_type CHAR(1), OUT num INTEGER, OUT sql_code INTEGER, OUT err_msg CHAR(100))
LANGUAGE SQL
  BEGIN
    ...
    ELSE
      ...
      SET err_msg = '-';
      SET num = (SELECT Number FROM p2.account WHERE ID = p_id AND Balance = p_bal AND Type = p_type);
      SET sql_code = 0;
    END IF;
END@
--
